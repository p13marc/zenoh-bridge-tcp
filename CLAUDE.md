...
```bash
cargo build --release --features tls-termination  # tls-termination gates TLS-terminating listeners (--listen ...,cert=,key=)
...
- **L7 parsing is delegated to the [`flowscope`](https://crates.io/crates/flowscope) crate** (features `http` + `tls` + `http2` always — pure-compute, no async/CAP/root; `http2` moved to the base build in #74, since the auto-detect door routes plaintext h2c in every build). `flowscope::classify::classify_first_bytes` detects the protocol; `flowscope::tls::TlsParser` extracts SNI (reassembling segmented/PQ ClientHellos); `flowscope::http::HttpProxyParser` frames HTTP/1.1 streaming — method-aware bodies, chunked, `1xx` interim, upgrades, and RFC 9112 §6.3 smuggling defense (`SmugglingPolicy::Strict` → typed `HttpPoison` → 400/502). Two doors reach the h2 head reader (`read_h2_head`): a terminating listener whose ALPN negotiated `h2` (require_preface=false), and the auto-detect door after classify saw the client preface (h2c, require_preface=true, #74 — with a timeout fallback to opaque relay for clients that wait for the server's SETTINGS). Either way `flowscope::http2::Http2Parser` peeks the first stream's `:authority`; plaintext HTTP/1 routes by `Host` via `read_http_head`. The bridge routes on the head and relays raw bytes it never retains — for h2 the multiplexed streams flow opaquely to one backend (single-authority proxy, not a per-stream demux). An h2 connection's response (terminated or h2c) is tapped read-only (`bridge_import_connection`'s `ResponseTap` → `connection::h2_response_tap`) to surface each stream's gRPC status (`grpc-status` trailers, or Trailers-Only) as the `zbridge_grpc_status_total{service,code}` metric + a log line.
...
- `src/import/accept.rs` is the one accept loop behind every listener: per-connection tasks in a `JoinSet`, semaphore backpressure (D3), idle reaping (D5), graceful drain on shutdown
